...
export function historyToGeo(trajectory: AssetTrajectory | null | undefined): FeatureCollection<LineString,TrajectoryProperties>{
    if(!trajectory){
        return emptyTrajectory()
    }
    const coords: [number, number][] = trajectory.history.map((point)=> [point.longitude, point.latitude])
...
export function predictionToGeo(trajectory: AssetTrajectory | null | undefined): FeatureCollection<LineString,TrajectoryProperties>{
