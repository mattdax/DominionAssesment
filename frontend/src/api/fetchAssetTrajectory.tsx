...
export async function fetchAssetTrajectory(assetId: string, signal: AbortSignal): Promise<AssetTrajectory>{
    const response = await fetch("http://"+SERVER_HOST+":"+SERVER_PORT+"/api/assets/"+assetId+"/trajectory", {signal})
    if(!response.ok){
